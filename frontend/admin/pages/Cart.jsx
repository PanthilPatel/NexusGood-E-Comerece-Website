              ...
              <div key={item._id} className="glass-card p-6 flex flex-col md:flex-row items-center gap-8 group animate-fade-in">
                ...
                <div className="flex-1 space-y-4 text-center md:text-left">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-slate-500 uppercase tracking-widest font-bold">{item.category?.name || 'Item'}</p>
                  </div>
                  
                  <div className="flex items-center justify-center md:justify-start gap-6">
                    <div className="flex items-center bg-white/5 border border-white/5 rounded-lg p-0.5">
                      <button onClick={() => updateQuantity(item._id, Math.max(1, item.quantity - 1))} className="p-1.5 text-slate-400 hover:text-white transition-colors"><Minus size={14} /></button>
                      <span className="w-8 text-center text-sm font-bold">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item._id, item.quantity + 1)} className="p-1.5 text-slate-400 hover:text-white transition-colors"><Plus size={14} /></button>
                    </div>
                    <button onClick={() => removeFromCart(item._id)} className="text-accent hover:text-accent-light transition-colors"><Trash2 size={18} /></button>
                  </div>
                </div>

                <div className="text-right">
                   <p className="text-2xl font-bold text-white">₹{(item.price * item.quantity).toLocaleString()}</p>
                   <p className="text-xs text-slate-500 font-medium italic">₹{item.price.toLocaleString()} / unit</p>
                </div>
              </div>
            ))}

...
export default function Roles() {
   ...
   return (
      <div className="h-full flex flex-col relative overflow-hidden">
         {/* ── SCROLLABLE CONTENT ── */}
         <div className={`flex-1 overflow-y-auto custom-scrollbar p-8 transition-all duration-200 ${showElevationModal ? 'blur-sm pointer-events-none select-none' : ''}`}>
                           ...
                           {loading ? (
                              ...
                              <div key={u._id} className="p-8 flex items-center justify-between hover:bg-white/[0.01] transition-all group">
                                 <div className="flex items-center gap-6">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-outfit font-bold text-2xl border ${u.role === 'admin' ? 'bg-primary/10 border-primary/20 text-primary' : 'bg-amber-500/10 border-amber-500/20 text-amber-500'}`}>
                                       {u.name?.charAt(0)}
                                    </div>
                                    <div className="space-y-1">
            ...
            {showElevationModal && (
               ...
               >
                  ...
                  <div className="absolute inset-0" onClick={() => setShowElevationModal(false)} />

               {/* Modal Card - Strictly Centered */}
               <div className="relative w-full max-w-xl m-0 p-0 bg-[#020617] border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] transition-all duration-300">
                  {/* Fixed Header */}
                  <div className="p-10 pb-0 flex flex-col gap-8 flex-shrink-0">
                  ...
                  {/* Scrollable List */}
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-3 overscroll-contain [transform:translateZ(0)]">
                     {users
